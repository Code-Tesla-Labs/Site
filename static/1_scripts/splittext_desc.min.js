...
function create_lines() {
  var text = $(".split_lines");
  split = new SplitText(text);
  //split.lines;
  $(split.lines).each(function(t) {
        TweenMax.from($(this), 2.5, {
          opacity: 0,
          x: random(-500, 500),
          y: random(-500, 500),
...
function create_show_split() {
// элементы всплывают снизу
    var text = $(".create_show_split"),
    split = new SplitText(text);
  split.create(".create_show_split", {
    type: "words,lines",
    linesClass: "line",
    autoSplit: true,
    mask: "lines",
...
on('body', 'click', '.select_serve', function(event) {
  ...
  category_data_sum = _this.parentElement.parentElement.parentElement.parentElement.parentElement.querySelector(".active");
  console.log("category_data_sum", category_data_sum);
  category_data_sum_price = category_data_sum.getAttribute("data-sum")*1;

  if (!_this.classList.contains("hover")){
    // если до нажатия опция не выбрана...
    counter.innerHTML = counter.innerHTML*1 + serve_price;
    category_data_sum.setAttribute("data-sum", category_data_sum_price + serve_price);
...
on('body', 'click', '.show_web_service', function() {
  ...
  if (next_div.classList.contains("hidden")) {
    counter.innerHTML = counter.innerHTML*1 + next_div.querySelector(".active").getAttribute("data-sum")*1;
  } else {
    counter.innerHTML = counter.innerHTML*1 - next_div.querySelector(".active").getAttribute("data-sum")*1;
  }
